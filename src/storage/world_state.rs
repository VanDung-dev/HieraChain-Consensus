...
mod tests {
    ...
    fn test_world_state_creation() {
        let ws = WorldState::new("TestChain");
        assert_eq!(ws.chain_name, "TestChain");
    }

    #[test]
    fn test_entity_state_update() {
        let mut ws = WorldState::new("TestChain");

        let mut event_updates = HashMap::new();
        event_updates.insert("value".to_string(), "100".to_string());

        let event = Event {
            event: "creation".to_string(),
            entity_id: "entity1".to_string(),
            timestamp: 1000.0,
            updates: event_updates.clone(),
            new_status: None,
        };
        ...
        let state = ws.get_entity_state("entity1");
    ...
    fn test_bulk_update() {
        ...
        updates1.insert("status".to_string(), "active".to_string());

        let mut updates2 = HashMap::new();
        updates2.insert("status".to_string(), "inactive".to_string());

        let bulk_updates = vec![
            ("entity1".to_string(), updates1),
            ("entity2".to_string(), updates2),
        ];

        ws.bulk_update(&bulk_updates);
        assert_eq!(ws.count_entities(), 2);
    }
}
