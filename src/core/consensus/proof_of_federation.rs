...
mod tests {
    ...
    #[test]
    fn test_new_proof_of_federation() {
        let pof = ProofOfFederation::new("TestPoF");
        assert_eq!(pof.name, "TestPoF");
        assert!(pof.validators.is_empty());
        assert_eq!(
            pof.config.get("block_interval").and_then(|v| v.as_f64()),
            Some(5.0)
        );
    }
    ...
    fn test_add_remove_validator() {
        let mut pof = ProofOfFederation::new("TestPoF");

        assert!(pof.add_validator("validator_a".to_string(), None));
        assert!(pof.add_validator("validator_b".to_string(), None));
        assert_eq!(pof.get_validator_count(), 2);

        // Duplicate should fail
        assert!(!pof.add_validator("validator_a".to_string(), None));

        // Remove
        assert!(pof.remove_validator("validator_b"));
        assert_eq!(pof.get_validator_count(), 1);
    }
    ...
    fn test_validators_sorted() {
        ...
        pof.add_validator("charlie".to_string(), None);
        pof.add_validator("alice".to_string(), None);
        pof.add_validator("bob".to_string(), None);

        assert_eq!(pof.validators, vec!["alice", "bob", "charlie"]);
    }

    #[test]
    fn test_get_current_leader() {
        let mut pof = ProofOfFederation::new("TestPoF");

        assert!(pof.get_current_leader(0).is_none());

        pof.add_validator("alice".to_string(), None);
        pof.add_validator("bob".to_string(), None);
        pof.add_validator("charlie".to_string(), None);

        assert_eq!(pof.get_current_leader(0), Some(&"alice".to_string()));
        assert_eq!(pof.get_current_leader(1), Some(&"bob".to_string()));
        assert_eq!(pof.get_current_leader(2), Some(&"charlie".to_string()));
        assert_eq!(pof.get_current_leader(3), Some(&"alice".to_string())); // wraps
    }

    #[test]
    fn test_validate_block_proposer() {
        let mut pof = ProofOfFederation::new("TestPoF");

        pof.add_validator("alice".to_string(), None);
        pof.add_validator("bob".to_string(), None);

        assert!(pof.validate_block_proposer(0, "alice"));
        assert!(!pof.validate_block_proposer(0, "bob"));
        assert!(pof.validate_block_proposer(1, "bob"));
    }
    ...
    fn test_can_create_block() {
        ...
        // Not enough validators (need 3)
        pof.add_validator("alice".to_string(), None);
        pof.add_validator("bob".to_string(), None);
        assert!(!pof.can_create_block(Some("alice")));

        // Add third validator
        pof.add_validator("charlie".to_string(), None);
        assert!(pof.can_create_block(Some("alice")));
        assert!(!pof.can_create_block(Some("unknown")));
    }
}
