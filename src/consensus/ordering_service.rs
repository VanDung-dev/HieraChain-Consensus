...
mod tests {
    ...
    #[test]
    fn test_ordering_node_health() {
        let node = OrderingNode {
            node_id: "node1".to_string(),
            endpoint: "http://localhost:7050".to_string(),
            is_leader: true,
            weight: 1.0,
            status: OrderingStatus::Active,
        ...
        assert!(node.is_healthy(30.0));
    }

    #[test]
    fn test_event_certifier() {
        let certifier = EventCertifier::new();
        ...
        let event = PendingEvent {
            ...
            submitter_org: "org1".to_string(),
            received_at: current_timestamp(),
            status: EventStatus::Pending,
            certification_result: None,
        };

        let result = certifier.validate(&event);
    ...
    fn test_block_builder() {
        ...
        let event1 = PendingEvent {
            event_id: "event1".to_string(),
            event_data: json!({"entity_id": "e1", "event": "test", "timestamp": current_timestamp()}),
            channel_id: "ch1".to_string(),
            submitter_org: "org1".to_string(),
            received_at: current_timestamp(),
            status: EventStatus::Certified,
            certification_result: None,
        };

        let event2 = PendingEvent {
            event_id: "event2".to_string(),
            event_data: json!({"entity_id": "e2", "event": "test", "timestamp": current_timestamp()}),
            channel_id: "ch1".to_string(),
            submitter_org: "org1".to_string(),
            received_at: current_timestamp(),
            status: EventStatus::Certified,
        ...
        assert!(builder.add_event(event1).is_none());
        let block = builder.add_event(event2);
